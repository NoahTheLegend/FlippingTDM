...
void onCollision(CBlob@ this, CBlob@ blob, bool solid)
{
    ...
    if (is_flipped && this.getPosition().y > blob.getPosition().y + 2)
    {
        float enemydam = 0.0f;
...
f32 onHit(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u8 customData)
{
    ...
    if (customData == Hitters::stomp && damage > 0.0f)
    {
        if ((is_flipped && velocity.y < 0.0f && worldPoint.y > this.getPosition().y) ||
            (!is_flipped && velocity.y > 0.0f && worldPoint.y < this.getPosition().y))
        {
