...
{
	this.setAngleDegrees(getRules().get_f32("global_rotation"));
	...
	if (is_flipped) this.SetFacingLeft(!this.isFacingLeft());
	...
	if (this.isAttached())
	{
		...
		if (att !is null)
		{
			...
			if (b !is null)
			{
				...
				if (is_flipped ? vel.y < -0.5f : vel.y > 0.5f)
				{
					b.AddForce(Vec2f(0, -20 * ff));
				}
			}
		}
	}
