...
void onTick(CSprite@ this)
{
	...
	bool inair = (!has_ground && !blob.isOnLadder());
	...
	Vec2f vec;
	int direction = blob.getAimDirection(vec) * ff;
	...
	if (chop !is null)
	{
		...
		if (wantsChopLayer)
		{
			...
			chop.ResetTransform();
			if (this.isFacingLeft())
				chop.RotateBy((is_flipped ? 0 : 180.0f) + chopAngle, Vec2f());
			else
				chop.RotateBy((is_flipped ? 180 : 0) + chopAngle, Vec2f());
		}
	}
...
{	
	...
	{
		bool is_flipped = getRules().get_bool("flipped");
		if (is_flipped) return; // todo: fix misaligned cursor
