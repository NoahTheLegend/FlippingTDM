...
f32 onHit(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u8 customData)
{
	...
	//if (hitterBlob !is null)
	{
		...
		if (is_flipped ? velocity.x < -0.7 : velocity.x > 0.7)
		{
			x_side = 1.0f;
		}
		else if (is_flipped ? velocity.x > 0.7 : velocity.x < -0.7)
		{
			x_side = -1.0f;
		}

		if (is_flipped ? velocity.y < -0.5 : velocity.y > 0.5)
		{
			y_side = 1.0f;
		}
		else
	...
	{
		this.AddForce(f * ff * 40.0f * scale * Maths::Log(2.0f * (10.0f + (damage * 2.0f))));
	}
