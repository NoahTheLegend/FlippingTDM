...
void onTick(CBlob@ this)
{
	...
	const bool inair = (!has_ground && !this.isOnLadder());

	Vec2f vec;

	const int direction = this.getAimDirection(vec) * ff;
	...
	if (shieldState)
	{
		...
		if (specialShieldState)
		{
			...
			{
				setShieldDirection(this, Vec2f(0, -1) * ff);
				setShieldAngle(this, SHIELD_BLOCK_ANGLE_GLIDING);
			}
			else //shield dropping
			{
				setShieldDirection(this, Vec2f(horiz, 2) * ff);
		...
		else if (walking)
		{
			...
			{
				setShieldDirection(this, Vec2f(horiz, 0) * ff);
			}
			else if (direction == 1)   //down
			{
				setShieldDirection(this, Vec2f(horiz, 3) * ff);
			}
			else
			{
				setShieldDirection(this, Vec2f(horiz, -3) * ff);
...
bool getInAir(CBlob@ this)
{
	...
	bool inair = (!has_ground && !this.isOnLadder());
	return inair;
}
...
bool getForceDrop(CBlob@ this, RunnerMoveVars@ moveVars)
{
	...
	bool forcedrop = ((is_flipped ? vel.y < Maths::Max(Maths::Abs(vel.x), 2.0f) * ff : vel.y > Maths::Max(Maths::Abs(vel.x), 2.0f)) &&
...
class ShieldGlideState : KnightState
{
	...
	bool TickState(CBlob@ this, KnightInfo@ knight, RunnerMoveVars@ moveVars)
	{
		...
		else
		{
			...
			{
				this.AddForce(Vec2f(0, -20.0f * glide_amount * ff));
			}
...
class SwordDrawnState : KnightState
{
	...
	bool TickState(CBlob@ this, KnightInfo@ knight, RunnerMoveVars@ moveVars)
	{
		...
		if (knight.swordTimer >= KnightVars::slash_charge_limit)
		{
			...
			if (this !is null) setKnocked(this, 15);
			knight.state = KnightStates::normal;
		}
...
class SlashState : KnightState
{
	...
	bool TickState(CBlob@ this, KnightInfo@ knight, RunnerMoveVars@ moveVars)
	{
		...
		{
			f32 md = KnightVars::slash_move_max_speed;
			if ((Maths::Abs(vel.x) < md && vel.y > -md))
			{
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid, Vec2f normal, Vec2f point1)
{
	...
	if (this.getShape().vellen > SHIELD_KNOCK_VELOCITY || has_ground)
	{
		KnightInfo@ knight;
		if (!this.get("knightInfo", @knight))
		{
